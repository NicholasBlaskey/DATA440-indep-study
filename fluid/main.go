...
var config = struct {
	...
	SIM_RESOLUTION:       512, //512,
	DYE_RESOLUTION:       1024,
	CAPTURE_RESOLUTION:   512,
	DENSITY_DISSIPATION:  1.3,  //1.0,
	VELOCITY_DISSIPATION: 0.74, //0.0
	PRESSURE:             0.8,
	PRESSURE_ITERATIONS:  20, //20,
	CURL:                 30.0,
	SPLAT_RADIUS:         2.5,  //0.75, //0.85,
...
func applyInputs(programs *shaders, fbos *framebuffers) {
	if pointer.moved {
		...
		brightScale := float32(0.8)

		/*
			col := mgl.Vec3{0.0, 0.0, 0.0}
			t := int(glfw.GetTime())
			if t%2 == 0 {
				col[2] = 0.25
			} else {
		...
		*/
		...
		splat(programs, fbos, pointer.texcoordX,
			...
			//mgl.Vec3{0 * rand.Float32() * brightScale,
...
func multipleSplats(programs *shaders, fbos *framebuffers, n int) {
	...
	cols := []mgl.Vec3{
		mgl.Vec3{maxCol, minCol, minCol},
		mgl.Vec3{minCol, maxCol, minCol},
		mgl.Vec3{minCol, minCol, maxCol},
	}
